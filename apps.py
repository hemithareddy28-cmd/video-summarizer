...
with right:
    ...
    if r:
        ...
        with T2:
            ts = r.get("timestamps",[])
            if ts:
                st.markdown('<div class="lbl">Jump to segment</div>', unsafe_allow_html=True)
                seg_labels = [f'[{s["time"]}]  {s["text"][:65]}' for s in ts]
                sel = st.selectbox("Pick segment", options=list(range(len(seg_labels))),
                                   format_func=lambda i: seg_labels[i],
                                   label_visibility="collapsed", key="seg_sel")
                if st.button("▶ Play from this point", use_container_width=True, key="play_btn"):
                    st.session_state.seek = ts_to_sec(ts[int(sel)]["time"])
                    st.rerun()
                st.markdown('<div class="lbl" style="margin-top:.6rem">All Segments</div>', unsafe_allow_html=True)
                rows = "".join(f'<div class="tr"><div class="tt">{s["time"]}</div><div class="tx">{s["text"]}</div></div>' for s in ts)
        ...
        with T4:
            ...
            if hist:
                ...
                for m in hist:
                    ...
                    bubbles += f'<div class="cb {cls}">{m["content"]}</div>'
                st.markdown(f'<div class="cw">{bubbles}</div>', unsafe_allow_html=True)
            else:
                st.markdown('<div class="ce">💬 Ask anything about this video…</div>', unsafe_allow_html=True)
            ...
            if not hist:
                ...
                for col,q in zip([c1,c2,c3],["What is this about?","Key takeaways?","How does it start?"]):
                    with col:
                        if st.button(q, use_container_width=True, key=f"q_{q[:6]}"):
            ...
            if st.session_state.pending_q:
                ...
                with st.spinner("Thinking…"):
                    ans = answer_question(transcript, q, chat_history=st.session_state.chat, api_key=st.session_state.apikey)
